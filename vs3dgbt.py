...
vGBT-0.9.60 [NINE-ITEM BUILD — USER 08-25, all sanctioned]
  • CLEAN-SIGNED BOOK: partition mode REMOVED (user verdict: naive in
    disguise); Book toggle = flow-signed / clean-signed — clean renders
    verdict×γ·OI ONLY where the quarantined single-leg tape spoke (no verdict
    = no bar; blankness is the feature). Highlight overlay deleted.
  • BAR-LEVEL LADDER: rung 1 = near-side meaningful bar (≥20% of own peak) of
    the nearest non-thin cluster; rung 2 = biggest bar strictly beyond, SAME
    CLUSTER ALLOWED — Dan's X≫Y within structures; single-rung sides dead.
    Wall standoff fires only when the rung sits on its cluster's peak bar.
    Fixtures without bar detail keep legacy peak behavior.
  • MARKET READ: Read+Signals merged; Signals lives in a collapsed diagnostics
    expander (charm-gate settings preserved); METHOD card renders as its OWN
    white JPG + download button for slide comparison.
  • REPLAY STATE MACHINE: LIVE/PLAYING/PAUSED · ⏮ ▶⏸ ⏹ · end→PAUSED (no
    wrap-loop) · recording continues silently in background · replay display
    fully frozen (no spinner/live override) · handler rerun-loop class removed.
  • CAP AUTOPILOT: 2 consecutive frames beyond 1.5× (or under 0.4×) auto-
    recalibrate to 1.3× recent mean, logged on the caption; stale nag deleted.
  • GEX³: absolute ± % price-room slider (default 0.5%) replaces the zoom
    multiplier. Live price layer defaults 30s (15s glitched redraws).
    Combined VS3D tab always on (gate obsolete since 0.9.41 memoization).

vGBT-0.9.59 [WHITE DOT REMOVED — USER 08-17]
  • Book dots: blue market-open dot only (yesterday's standing book, t−1
    reference). White prev-snap dot deleted both paths.
...
import contextlib, time as _time, warnings
...
def signed_clusters(rows, lo, hi, min_share=0.05):
    ...
    for c in out:
        ...
        bars=list(zip([float(x) for x in ks],[float(x) for x in w]))   # 0.9.60: bar detail for the ladder
        conf=float((np.array(c["cs"])*w).sum()/m)
        share=m/tot
        if share>=min_share: res.append(dict(sign=c["sign"],peak=peak,pmass=pmass,mass=m,conf=conf,share=share,bars=bars))
    return res
    ...
    if lock:
        ...
        def _tests(side):                               # side=+1 above the open, -1 below
            ...
            if _inw:
                ...
                # 0.9.60 BAR-LEVEL LADDER [USER 08-25 + SLIDES 08-04 7620≫7635 ·
                # 07-30 7410≫7445 · 07-02 7515≫7525]: rung 1 = the NEAR-SIDE
                # meaningful bar (≥20% of the cluster's own peak bar) of the
                # nearest non-thin cluster — the engagement face, not the peak;
                # rung 2 (below) = biggest bar strictly beyond, SAME CLUSTER
                # ALLOWED. Fixtures without bar detail keep peak behavior.
                if t1.get("bars"):
                    _fb=[(k,v) for k,v in t1["bars"] if v>=0.20*t1["pmass"]]
                    if _fb: t1["peak"]=float(min(_fb,key=lambda kv:abs(kv[0]-ref))[0])
            ...
            # 0.9.60: candidate BARS strictly beyond rung 1 — same cluster allowed
            # (fixes single-rung sides + Dan's within-structure X≫Y). Cluster-level
            # fallback when bar detail is absent (legacy fixtures).
            _bb=[(k,v,c) for c in _inw for k,v in (c.get("bars") or [])
                 if side*(k-t1["peak"])>0 and v>=0.20*c.get("pmass",v)]
            if _crazy:                                   # "crazy in size" displaces the outer rung
                t2=dict(min(_crazy,key=lambda c:side*(c["peak"]-ref))); t2["ext"]=True
            elif _bb:
                _k2,_v2,_c2=max(_bb,key=lambda t:t[1])
                t2=dict(_c2); t2["peak"]=float(_k2)
            elif _abv_in:
            ...
            for c in out:                                # thin = under 20% of in-window side max
                ...
                _cmpr=max(c["mass"]-_pm,_oth)
                _onpeak=(not c.get("bars")) or any(abs(k-c["peak"])<1e-9 and abs(v-_pm)<1e-9 for k,v in c["bars"])
                if _onpeak and _pm>0 and _cmpr>0 and _pm>=WALL_X*_cmpr:
                    c["wall_at"]=c["peak"]; c["peak"]=c["peak"]-side*5.0; c["wall"]=True
...
def signed_book_rows(ch, sp, mode="flow", doi=None):
    ...
    if mode=="flow" and not ch["dsign"].notna().any(): return None
    # clean mode builds from the verdict map alone; dsign column presence still gates flow mode
    cc=ch.copy()
    ...
    if mode=="clean":
        ...
        # legs; NO VERDICT → NO BAR (strike dropped — blankness is the feature:
        # what remains is where deliberate single-leg players positioned).
        # ΔOI-partitioned mode REMOVED same build (user verdict: naive in
        # disguise; the edge lives in flow-signing).
        cm=doi or {}                                       # {strike: −1/0/+1}
        vv=np.array([float(cm.get(float(k),0.0) or 0.0) for k in cc["strike"]])
        SCp=100.0*sp*sp/10000.0
        cc["_v"]=vv*cc["gamma"].fillna(0)*cc["oi"].fillna(0)*SCp
        g=cc.groupby("strike",as_index=False).agg(signed_pct=("_v","sum"))
        g=g[g["signed_pct"]!=0.0]                          # no verdict → no bar
        g["conf"]=1.0                                      # deliberate tape = full opacity
...
def book_figure(book,spot,straddle,lo,hi,side="Total",prev=None,openb=None,signed=None,
    ...
    if signed is not None and len(signed):
        ...
        # 0.9.59 [USER 08-17]: white prev-snap dot removed — blue open dot only
        # (= yesterday's standing book, our t−1 reference).
        if _op: ax.scatter(list(_op.values()),list(_op.keys()),s=14,color="#6f9bd1",zorder=4,label="market open")
    ...
    _dots(openb,"#6f9bd1","market open")   # 0.9.59: white prev-snap dot removed
...
_live_pick=st.sidebar.selectbox("📡 Live price layer (TV poll)",["15s","30s","60s","off"],index=1,   # 0.9.60: 30s default — 15s glitched redraws
...
b_signmode=st.sidebar.radio("Book signing",["flow-signed (current)","clean-signed (single-leg tape)"],
    ...
    help="0.9.60: clean-signed renders the book PURELY from the quarantined single-leg tape — verdict × γ·OI, "
         ...
         "sweep (day-cached). METHOD lock/strip stay flow-signed.")
...
with st.sidebar.expander("📊 Book controls", expanded=False):
    ...
    b_clean=False   # 0.9.60 [USER 08-25]: highlight overlay removed — the pure clean-signed book mode replaces it
...
try:
    ...
    if st.session_state.get("pb_mode")=="PLAYING" and st.session_state.get("frames"):
...
def _due():
    # 0.9.60: recording CONTINUES during replay (silent, background) — only the
    # DISPLAY is frozen; the film keeps growing while you review.
...
if force or refresh or _due():
    _sil=st.session_state.get("pb_mode","LIVE")!="LIVE"
    with (st.spinner("Taking chain snapshot…") if not _sil else contextlib.nullcontext()):
        try: take_snapshot(num_expiries)
...
# 0.9.60 REPLAY STATE MACHINE [USER 08-25]: pb_mode ∈ LIVE / PLAYING / PAUSED —
# one source of truth. ⏮ Rewind · ▶/⏸ Play-Pause · ⏹ Stop (exit to LIVE).
# End of film → auto-PAUSED at the last frame (no wrap-loop, no stuck state).
# During ANY replay state, recording continues silently in the background (no
# spinner, no visual override) and live fragments are hard-disabled.
if "pb_mode" not in st.session_state: st.session_state.pb_mode="LIVE"
...
st.session_state.pb_play=(st.session_state.pb_mode=="PLAYING")   # legacy alias for downstream gates
st.session_state.pb_follow=(st.session_state.pb_mode=="LIVE")
st.sidebar.markdown("---"); st.sidebar.markdown("**▶ Playback (recorded snapshots)**")
_frame_ts=[s2["ts"] for s2 in snaps if s2["ts"].isoformat() in st.session_state.frames]
_nframes=len(_frame_ts)
if _nframes==0:
    st.sidebar.caption("No cached frames yet — they record automatically each snapshot.")
    PLAYBACK=False
else:
    _m=st.session_state.pb_mode
    c1,c2,c3=st.sidebar.columns(3)
    if c1.button("⏮",use_container_width=True,help="Rewind to frame 1 (paused)"):
        st.session_state.pb_idx=0; st.session_state.pb_mode="PAUSED"
    if c2.button(("⏸" if _m=="PLAYING" else "▶"),use_container_width=True,help="Play / pause the film"):
        if _m=="PLAYING": st.session_state.pb_mode="PAUSED"
        else:
            if st.session_state.pb_mode=="LIVE": st.session_state.pb_idx=0
            st.session_state.pb_mode="PLAYING"; st.session_state.pb_last_tick=None
    if c3.button("⏹",use_container_width=True,help="Stop — back to LIVE (latest, live layers resume)"):
        st.session_state.pb_mode="LIVE"; st.session_state.pb_idx=_nframes-1
    _m=st.session_state.pb_mode
    if _m=="PLAYING":
        ...
        if _tick is not None and _tick!=_lt:
            if _lt is not None:
                if st.session_state.pb_idx>=_nframes-1:      # end of film → PAUSED, never wrap
                    st.session_state.pb_mode="PAUSED"
                else:
        ...
        st.session_state.pb_idx=st.sidebar.slider("Frame",0,_nframes-1,
            int(min(st.session_state.pb_idx,_nframes-1)))
        st.sidebar.caption(f"paused · ⏹ returns live · recording continues ({_nframes} frames)")
    else:
        st.session_state.pb_idx=_nframes-1                    # LIVE follows the latest frame
    ...
    PLAYBACK=(st.session_state.pb_mode!="LIVE")   # 0.9.60: any replay state freezes the display
    if PLAYBACK:
...
combo_on=True   # 0.9.60 [USER 08-25]: combined always on — gate obsolete since 0.9.41 memoization
...
def book_sign_args():
    """0.9.60: (mode, payload) for the DISPLAY pipeline. clean → verdict map
    from the existing budgeted clean sweep (day-cached); missing → {} (blank
    book with honest caption)."""
    if not str(st.session_state.get("b_signmode","")).startswith("clean"):
        return "flow",None
    m={}
    try:
        m=gbt_clean_signs(exps[0],[float(k) for k in sorted(set(latest["chain"]["strike"]))] ) or {}
    except Exception: m={}
    if not m: st.caption("clean sweep pending/empty — flip again or wait for the budgeted sweep to finish")
    return "clean",m
...
with tab_terr:
    ...
    def _render_terrain():
        ...
        # 0.9.60 CAP AUTOPILOT [USER 08-25 "why can't you automate it"]:
        # stale/dead caps self-heal — 2 consecutive frames beyond 1.5× (or under
        # 0.4×) recalibrate to 1.3× recent-mean, announced on the caption, never
        # nagged. Buttons remain manual override.
        if t_norm=="Manual (fixed cap)" and used_cap:
            _bad=(_p92>1.5*used_cap) or (_p92<0.40*used_cap)
            _ck60=f"cap_bad_{t_greek}_{t_wt}"
            st.session_state[_ck60]=st.session_state.get(_ck60,0)+1 if _bad else 0
            if st.session_state[_ck60]>=2:
                _h60=st.session_state.get(f"terr_hist_{t_greek}_{t_wt}",[])[-24:]
                if _h60:
                    st.session_state[capkey]=1.3*float(np.mean(_h60)); st.session_state[_ck60]=0
                    st.session_state.setdefault("cap_log",[]).append(f"{now_naive:%H:%M} ×{_p92/used_cap:.1f}")
        _cl60=st.session_state.get("cap_log",[])
        if _cl60: st.caption("cap auto-recalibrated: "+" · ".join(_cl60[-4:]))
        _t=now_naive.time()
...
with tab_read:
    with st.expander("🧭 diagnostics (legacy Signals — charm gate settings live here)",expanded=False):
...
with tab_gex3:
    ...
    _g3sig=repr((sel_ts.isoformat(),sel_i,round(window_pct,4),_g3w,_g3s,round(_g3room,4),round(_g3wd,2),round(_g3h,1)))
